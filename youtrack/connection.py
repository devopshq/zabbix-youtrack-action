...
class Connection(object):
    ...
    def _login(self, login, password):
        response, content = self.http.request(
            self.baseUrl + "/user/login?login=" + urllib.parse.quote_plus(login) + "&password=" + urllib.parse.quote_plus(password),
            'POST',
    ...
    def _req(self, method, url, body=None, ignoreStatus=None, content_type=None):
        ...
        # content = content.translate(str.maketrans(None, '\0'))
    ...
    def _reqXml(self, method, url, body=None, ignoreStatus=None):
        ...
        if method == 'PUT' and ('location' in list(response.keys())):
    ...
    def updateIssue(self, issueId, summary, description):
        self._req('POST', "/issue/{issueId}?summary={summary}&description={description}".format(
            issueId=issueId,
            summary=urlquote(summary),
    ...
    def createIssue(self, project, assignee, summary, description, priority=None, type=None, subsystem=None, state=None,
                    affectsVersion=None,
                    fixedVersion=None, fixedInBuild=None, permittedGroup=None, service=None):
